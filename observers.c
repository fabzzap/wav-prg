...
static struct wav2prg_observed* get_observed(const char *observed_name, enum wav2prg_bool add_if_missing){
...
static void add_one_observed(const char *observer_name, const char *observed_name, wav2prg_recognize_block recognize_func){
  ...
  else if (!strcmp(observed_name, "kdc"))
  {
    ...
    struct wav2prg_observed *current_observed = get_observed(observed_name, wav2prg_true);
...
struct wav2prg_observed_loaders* get_observers(const char *observed_name){
  struct wav2prg_observed *current_observed = get_observed(observed_name, wav2prg_false);
  return current_observed ? current_observed->observers : NULL;
}
